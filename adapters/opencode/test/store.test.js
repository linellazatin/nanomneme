...
test('runMemory delegates retain and recall to a project database', () => {
  const project = temporaryDirectory('nmnm-opencode-project-');
  try {
    const retained = runMemory({
      cwd: project,
      store: 'project',
      operation: 'retain',
      input: { content: 'OpenCode adapter memory' },
    });
    ...
    assert.equal(recalled.content, 'OpenCode adapter memory');
...
test('runMemory gives global retained records the global scope by default', () => {
  ...
  try {
    const retained = runMemory({
      ...
      store: 'global',
      operation: 'retain',
      input: { content: 'Global OpenCode adapter memory' },
    });
