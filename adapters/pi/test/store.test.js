...
test('runMemory delegates retain and recall to a project database', () => {
  const project = temporaryDirectory('nmnm-pi-project-');
  try {
    const retained = runMemory({
      cwd: project,
      store: 'project',
      operation: 'retain',
      input: { content: 'Pi adapter memory' },
    });
    ...
    assert.equal(recalled.content, 'Pi adapter memory');
...
test('runMemory gives global retained records the global scope by default', () => {
  const project = temporaryDirectory('nmnm-pi-project-');
  const home = temporaryDirectory('nmnm-pi-home-');
  try {
    const retained = runMemory({
      cwd: project,
      home,
      ...
      store: 'global',
      operation: 'retain',
      input: { content: 'Global Pi adapter memory' },
    });
